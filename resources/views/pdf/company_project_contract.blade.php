            ...
            <tbody>
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <h6 style="text-align: justify">{{ $project->social_name }}, inscrito no CNPJ sob nº {{ $project->cnpj }}, adiante denominada apenas <b>VENDEDOR(A)</b> autoriza a intermediação pela {{ $company->name }}, <i>inscrita no CNPJ sob nº {{ $company->cnpj }} e CRECI {{ $company->creci }}</i>, adiante denominada apenas <b>INTERMEDIADOR(A)</b> para promover a venda da incorporação imobiliária de sua propriedade denominada, {{ $project->name }}, com incorporação devidamente {{ $project->notes }}.</h6>
                        <br>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <h6 style="text-align: justify">1ª – O valor de venda, formas de pagamentos e espelho de vendas deverá ser observado no sistema <b>MG2 – INCORP</b>, qual deve ser acessado pelo endereço www.mg2incorp.com.br com seu usuário e senha. Da mesma forma, as reservas só serão aceitas pelo sistema, qual se expiram automaticamente em {{ $project->expiration_time }} horas, caso não seja incluídos documentos necessários para analise e elaboração contratual.</h6>
                        <br>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <h6 style="text-align: justify">2ª – Pelos serviços prestados ora pactuados o (a) vendedor (a) pagará à imobiliária, desde que ocorra regular e efetiva compensação do pagamento a titulo de “Sinal” e “Principio de Pagamento” a título de comissão de corretagem imobiliária equivalente a {{ formatMoney($project->comission) }}% do valor efetivo da venda do imóvel qual será repassado a <b>INTERMEDIADOR(A)</b>.</h6>
                ...
                <tr>
                    <td colspan="2">
                        <h6 style="text-align: right">{{ $project->local }}, <?php setlocale(LC_TIME, 'pt_BR', 'pt_BR.utf-8', 'pt_BR.utf-8', 'portuguese'); echo utf8_encode(strftime('%d de %B de %Y', strtotime('today'))); ?></h6>
                        <br>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <br>
                        <h6 style="text-align: center"><b>{{ $project->social_name }}</b></h6>
                        <br>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <h6 style="text-align: justify">Este documento foi gerado por <b>MG2 – INCORP</b> e <b>ASSINADO DIGITALMENTE</b> por {{ @$project->constructor->name }}, sob nº: {{ @$company_project->code }} expedido em {{ formatData(date('Y-m-d')) }}. A aceitação desta autorização condiciona-se à verificação de sua autenticidade no portal do MG2 – Incorp (http://www.mg2incorp.com.br/validacao).</h6>
                    </td>
                </tr>
            </tbody>
        </table>
